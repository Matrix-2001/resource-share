...
const DISCOURAGED_TERMS = [
  term(0x6210, 0x4eba),
  term(0x76d7, 0x7248),
  term(0x7248, 0x6743, 0x4e89, 0x8bae),
  term(0x7248, 0x6743, 0x7ea0, 0x7eb7),
  term(0x4e0d, 0x826f, 0x4fe1, 0x606f)
]
...
`)
    ...
    assert.match(overview, /## 创作者内容索引/)
    assert.match(overview, /### \[Kemono\]\(\.\/creator-archives\/kemono\.md\)/)
    assert.match(creatorArchives, /# 创作者内容索引/)
...
`)
    ...
    assert.match(overview, /### \[示例特定内容资源\]\(\.\/❤大家想看的东西❤\/example-adult\.md\)/)
    assert.match(adult, /# ❤大家想看的东西❤/)
    assert.match(adult, /## \[示例特定内容资源\]\(\.\/example-adult\.md\)/)
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})

test('项目说明文案避免使用高风险描述词', async () => {
  const files = [
    'README.md',
    'AGENTS.md',
    'docs/index.md',
    'docs/contribute.md',
    'scripts/generate-resource-indexes.mjs',
    'docs/resources/index.md',
    'docs/resources/ebooks/z-library.md',
    'docs/resources/torrents/1337x.md',
    'docs/resources/torrents/the-pirate-bay.md',
    'docs/resources/torrents/index.md',
    'docs/resources/games/fit-girl.md',
    'docs/resources/games/oldman-emu.md',
    'docs/resources/games/privateers-club.md',
    'docs/resources/creator-archives/kemono.md',
    ...
    'docs/resources/❤大家想看的东西❤/theporndude.md',
  ...
  for (const file of files) {
    ...
    for (const term of DISCOURAGED_TERMS) {
      assert.equal(content.includes(term), false, `${file} 不应包含高风险描述词：${term}`)
    }
  }
})
